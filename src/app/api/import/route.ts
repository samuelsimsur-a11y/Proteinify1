...
Video description: ${yt.description}`;
      ...
      if (!parsed) {
        return NextResponse.json({ error: "Failed to parse imported recipe." }, { status: 500 });
      }
      return NextResponse.json(toImportSuccess("youtube", yt.title, parsed));
    }

    if (isTikTokUrl(url)) {
      const tk = await fetchTikTokCaption(url);
      if (!tk) {
        return NextResponse.json({ error: "TikTok video not found." }, { status: 404 });
      }
...
Caption: ${tk.caption}`;
      ...
      return NextResponse.json(toImportSuccess("tiktok", tk.caption, parsed));
    }

    return NextResponse.json(
      { error: "Unsupported URL. Paste a TikTok or YouTube link." },
      { status: 400 }
    );
  } catch (err) {
