#!/bin/bash
# Paste this entire script into your terminal inside the wisedish folder.
...
const BASE = `
You are Wise Dish — a culinary transformation engine that upgrades dishes people already love.
...
cat > src/lib/culinary/dil/data/wisedish_schema.json << 'ENDOFFILE'
{
  "name": "wisedish_compact_result",
  "strict": true,
  "schema": {
    "type": "object",
...
// Wise Dish generation route — DIL wired in.
//
// Flow:
...
import wisedishSchema from "@/lib/culinary/dil/data/wisedish_schema.json";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      response_format: {
        type: "json_schema",
        json_schema: wisedishSchema as Parameters<
