...
import type { WiseDishResponse, RecipeVersion } from "./types";
import {
 ...
 * Progressive `version` events may omit USDA enrichment; `complete` matches `WiseDishResponse`.
 */
export async function streamGenerateFull(
  ...
  if (!ct.includes("text/event-stream") || !res.body) {
    ...
    return parseWiseDishResponseJson(json);
  }
  ...
  const dispatchSse = (ev: string, data: string) => {
    if (ev === "version") {
      ...
      try {
        finalResult = parseWiseDishResponseJson(JSON.parse(data));
      } catch {
