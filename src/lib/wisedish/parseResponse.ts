...
export type ParseResult =
  ...
  | {
      ...
      classificationFull?: WiseDishResponse["classificationFull"];
    }
  | { ok: false; error: string };

export function isParseSuccess(r: ParseResult): r is { ok: true; data: WiseDishResponse } {
  ...
  return {
    ...
    classificationFull:
      json.classificationFull && typeof json.classificationFull === "object"
        ? (json.classificationFull as WiseDishResponse["classificationFull"])
        : undefined,
  };
}

/**
 * Validates the wire JSON matches the strict WiseDish contract (shape + version ids/labels).
 */
...
export function parseWiseDishResponseJson(json: unknown): ParseResult {
