 ...
 * Shapes match {@link parseWiseDishResponseJson} / {@link parseSingleVersionAiJson} — same wire fields as prompts (summary, macros.p/d, ingredients.swapOptions, etc.).
 */
const swapTypeEnum = {
  type: "string",
  enum: [
...
/** Full 3-version generate — matches wire shape validated by parseWiseDishResponseJson. */
export const wisedishFullResponseFormat: OpenAI.ResponseFormatJSONSchema = {
  type: "json_schema",
  json_schema: {
    name: "wisedish_result",
    strict: true,
    description:
...
export const wisedishSingleVersionResponseFormat: OpenAI.ResponseFormatJSONSchema = {
  type: "json_schema",
  json_schema: {
    name: "wisedish_single_version",
    strict: true,
    description:
