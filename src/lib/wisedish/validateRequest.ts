...
function isTransformationMode(x: unknown): x is TransformationMode {
  return x === "wisedish" || x === "proteinify" || x === "lean";
}

function normalizeTransformationMode(
  mode: unknown
): TransformationMode | undefined {
  if (mode === "proteinify") return "wisedish";
  if (mode === "wisedish" || mode === "lean") return mode;
  return undefined;
}
...
export function validateGenerateRequestBody(
  ...
  const transformationModeRaw = o.transformationMode;
  if (transformationModeRaw !== undefined && !isTransformationMode(transformationModeRaw)) {
    return { ok: false, error: 'transformationMode must be "wisedish" or "lean".' };
  }
  const transformationMode = normalizeTransformationMode(transformationModeRaw);
